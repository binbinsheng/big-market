...
import com.binbinsheng.domain.activity.event.ActivityZeroSkuStockMessageEvent;
...
public class ActivityRepository implements IActivityRepository {
    ...
    @Resource
    private ActivityZeroSkuStockMessageEvent activityZeroSkuStockMessageEvent;
    ...
    public void doSaveOrder(CreateQuotaOrderAggregate createQuotaOrderAggregate) {
        RLock lock = redisService.getLock(Constants.RedisKey.ACTIVITY_ACCOUNT_LOCK
                + createQuotaOrderAggregate.getUserId() + Constants.UNDERLINE + createQuotaOrderAggregate.getActivityId());
        try {
            lock.lock(3, TimeUnit.SECONDS);
            // 订单对象
            ActivityOrderEntity activityOrderEntity = createQuotaOrderAggregate.getActivityOrderEntity();
            //将activityOrderEntity转换为数据库持久化对象
            ...
            // 以用户ID作为切分键，通过 doRouter 设定路由【这样就保证了下面的操作，都是同一个链接下，也就保证了事务的特性】
                ...
                transactionTemplate.execute(status -> {
                    try {
                        ...
                        // 2. 更新总账户(如果返回0，说明不存在账户)
                        int count = raffleActivityAccountDao.updateAccountQuota(raffleActivityAccount);
                        // 3. 创建账户 - 更新为0，则账户不存在，创新新账户。
                        if (0 == count) {
                            //另一个线程可能也会插入，导致主键冲突，走catch
    ...
    public boolean subtractionActivitySkuStock(Long sku, String cacheKey, Date endDateTime) {
        ...
        if (surplus == 0) {
            //库存消耗没了以后，发送MQ消息，直接清空数据库库存
            eventPublisher.publish(activityZeroSkuStockMessageEvent.topic(),
                    activityZeroSkuStockMessageEvent.buildEventMessage(sku));
            return false;
        }else if (surplus < 0){//并发导致库存更新为负数，设置为0
    ...
    public void saveCreatePartakeOrderAggregate(CreatePartakeOrderAggregate createPartakeOrderAggregate) {
        try{
            ...
            transactionTemplate.execute(status -> {
                try{
                    //1.更新用户账户信息
                    int totalCount = raffleActivityAccountDao.updateActivityAccountSubtractQuota(
                    ...
                    if (createPartakeOrderAggregate.isExistAccountMonth()){
                        int monthCount = raffleActivityAccountMonthDao.updateActivityAccountMonthSubtractQuota(
                    ...
                    if (createPartakeOrderAggregate.isExistAccountDay()){
                        int updateDayCount = raffleActivityAccountDayDao.updateActivityAccountDaySubtractQuota(RaffleActivityAccountDay.builder()
                                .userId(userId)
                                .activityId(activityId)
